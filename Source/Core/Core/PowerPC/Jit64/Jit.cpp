...
const u8* Jit64::DoJit(u32 em_address, PPCAnalyst::CodeBuffer *code_buf, JitBlock *b, u32 nextPC)
{
	...
	// If the block depends on an input register which looks like a gather pipe or MMIO related constant,
	// guess that it is actually a constant input, and specialize the block based on this assumption. This
	// happens when there are branches in code writing to the gather pipe, but only the first block loads
	// the constant.
	...
	if (js.noSpeculativeConstantsAddresses.find(js.blockStart) == js.noSpeculativeConstantsAddresses.end())
	{
